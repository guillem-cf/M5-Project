...
if __name__ == '__main__':
    ...
    db_dataset_val = TripletCOCORetrieval(val_path, object_image_dict, 
                                            transform, "database", val_labels_path)



    # ------------------------------- DATALOADERS --------------------------------
    db_train_loader = DataLoader(db_dataset_train, batch_size=1, shuffle=False,
                                    pin_memory=True, num_workers=10)
    db_val_loader = DataLoader(db_dataset_val, batch_size=1, shuffle=False,
                                    pin_memory = True, num_workers=10)
    ...
    start = time.time()
    train_embeddings_cl = metrics.extract_embeddings_coco(db_train_loader, model, device)
    end = time.time()
    print("Time to calculate train database embeddings: ", end - start)
    path = os.path.join(output_path, 'train_embeddings.png')
    metrics.plot_embeddings_coco(train_embeddings_cl, None, None, 'Database Embeddings', path)
    
    print("Calculating val database embeddings...")
    start = time.time()
    val_embeddings_cl = metrics.extract_embeddings(db_val_loader, model, device)
    end = time.time()
    print("Time to calculate val database embeddings: ", end - start)
    path = os.path.join(output_path, 'val_embeddings.png')
    metrics.plot_embeddings_coco(val_embeddings_cl, None, None, 'Validation Embeddings', path)
