...
df = pd.DataFrame(columns=['video_id', 'frame_id', 'class_id', 'obj_instance_id', 'path_frame_annotation', 'path_frame_image'])
    ...
    if os.path.isdir(video_path):
        ...
        for frame_id in os.listdir(video_path):
            ...
            if frame_path[-4:] == '.png':
                ...
                new_values = {'video_id': video_id, 'frame_id': frame_id, 'class_id': class_id, 'obj_instance_id': obj_instance_id, 'path_frame_annotation': frame_path, 'path_frame_image': frame_path_image}
                new_row = pd.DataFrame.from_records(new_values, index=[0])
                df = pd.concat([df, new_row])

                #print('Class id:', class_id)
                #print('Object instance id:', obj_instance_id)
